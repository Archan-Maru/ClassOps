...
function AcceptInvite() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-8 text-center">
        {status === "loading" && (
        ...
        {status === "invalid" && (
          <div className="flex flex-col items-center gap-4">
            ...
            <Link to="/dashboard" className="mt-2 text-sm text-indigo-400 hover:text-indigo-300">
              Go to Dashboard
            </Link>
          </div>
        )}
        ...
        {status === "already_accepted" && (
          <div className="flex flex-col items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-slate-100">Already Accepted</h2>
            <p className="text-sm text-slate-400">This invite has already been used.</p>
            <Link to="/dashboard" className="mt-2 text-sm text-indigo-400 hover:text-indigo-300">
              Go to Dashboard
            </Link>
          </div>
        ...
        {status === "need_login" && inviteInfo && (
          <div className="flex flex-col items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-slate-100">Class Invitation</h2>
            <p className="text-sm text-slate-300">
              <span className="font-medium text-indigo-300">{inviteInfo.invitedBy}</span> invited you to join
            </p>
            <p className="text-lg font-semibold text-slate-100">{inviteInfo.classTitle}</p>
            <p className="text-sm text-slate-400">Sign in or create an account to accept this invite.</p>
        ...
        {status === "ready" && inviteInfo && (
          <div className="flex flex-col items-center gap-4">
            ...
            <button
              ...
              className="mt-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-indigo-500"
            >
              Accept & Join Class
            </button>
          </div>
        ...
        {status === "accepting" && (
          <div className="flex flex-col items-center gap-3">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-indigo-400 border-t-transparent" />
            <p className="text-slate-400">Joining class...</p>
          </div>
        )}
        ...
        {status === "success" && (
          <div className="flex flex-col items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-slate-100">Joined Successfully!</h2>
            <p className="text-sm text-slate-400">Redirecting to class...</p>
          </div>
        )}
        ...
        {status === "error" && (
          <div className="flex flex-col items-center gap-4">
            ...
            <h2 className="text-xl font-bold text-slate-100">Failed to Join</h2>
